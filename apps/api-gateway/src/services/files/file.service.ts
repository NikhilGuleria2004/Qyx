...
export class FileService {
  ...
  async requestUploadUrl(organizationId: string, uploaderId: string, data: UploadUrl): Promise<{ fileId: string; uploadUrl: string; expiresIn: number }> {
    ...
    const r2Key = `${organizationId}/${fileId}`;
    
    await dbCreateFile(this.db, fileId, organizationId, uploaderId, r2Key, data.mime_type, data.size_bytes, data.conversation_id);
